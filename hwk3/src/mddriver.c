...
void driver(const int arg_count, char ** arg_list)
{
   ...
   int vals_per_proc = (int)(atoms.N / (float)nprocs);
   int start_index = rank*vals_per_proc;
   int end_index = (rank+1)*vals_per_proc;
   if(rank == nprocs - 1)
     end_index = atoms.N;

   atoms.start_index = start_index;
   atoms.end_index = end_index;
   atoms.nprocs = nprocs;
   atoms.rank = rank;

   //allocate recvcounts and displs (freed in atoms.c)
   atoms.recvcounts = (int *)(malloc(nprocs * sizeof(int)));
   atoms.displs = (int *)(malloc(nprocs * sizeof(int)));

   if(atoms.recvcounts == NULL || atoms.displs == NULL) {
   ...
   int i;
   for(i = 0; i < nprocs; ++i) {
     atoms.recvcounts[i] = vals_per_proc;
     atoms.displs[i] = i*vals_per_proc;
   }
   //adjust last recvcount
   atoms.recvcounts[nprocs-1] += (atoms.N - nprocs*vals_per_proc);

   if(rank == 0)
     for(i = 0; i < nprocs; ++i)
       printf("recvcount: %d       displ: %d\n",atoms.recvcounts[i],atoms.displs[i]);
